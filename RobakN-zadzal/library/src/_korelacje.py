...
def korelacje(dataset: dict, plot: bool = False) -> dict:
    ...
    #print(nhr_korelacja, hnr_korelacja, mupdrs_nhr_kor, mupdrs_hnr_kor, tupdrs_hnr_kor, tupdrs_hnr_kor)
    ...
    if plot:
        ...
        if opcja.lower() in ['y', 'yes', '']:
            ...
            m_updrs_f.plot(x="nhr", y="wartosc_MotorUPDRS", kind="scatter")
            plt.savefig(f"{filename}_mupdrs_nhr.png")
            m_updrs_f.plot(x="hnr", y="wartosc_MotorUPDRS", kind="scatter")
            plt.savefig(f"{filename}_mupdrs_hnr.png")
            t_updrs_f.plot(x="nhr", y="wartosc_TotalUPDRS", kind="scatter")
            plt.savefig(f"{filename}_tupdrs_nhr.png")
            t_updrs_f.plot(x="hnr", y="wartosc_TotalUPDRS", kind="scatter")
